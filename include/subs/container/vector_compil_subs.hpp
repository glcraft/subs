...
namespace subs
{
	...
	protected:
		...
		size_t getValue(biIterator name) const
		{
			...
			{
				std::cerr << __FUNCTION__ << " exception : Argument invalide. Argument pass� : " << name.str() << std::endl;
				return -2;
			}
			catch (std::out_of_range exc)
			{
				std::cerr << __FUNCTION__ << " exception : Nombre hors champ. Argument pass� : " << name.str() << std::endl;
				return -2;
			}
			return i;
		}
